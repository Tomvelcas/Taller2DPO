...
public class SandboxListas
{
    ...
    {
    	List<String> copia_cadena = new ArrayList<>();
    	for (String i : listaCadenas) {
    		copia_cadena.add (new String(i));
    	}
        return copia_cadena;
    }
    ...
    {
    	int posicion = 0;
    	while(posicion < listaEnteros.size()) {
    		if (listaEnteros.get(posicion) == valor) {
    ...
    {
    	int posicion = 0;
    	while(posicion < listaCadenas.size()) {
    		if(listaCadenas.get(posicion) == cadena) {
    ...
    public void volverPositivos( )
    {
    	for ( int contador=0; contador<listaEnteros.size(); contador++) {
    		if (listaEnteros.get(contador) < 0)
    		{
    		listaEnteros.set(contador, listaEnteros.get(contador) *(-1));
    		}
    	}
    }
    ...
    public int contarApariciones( String cadena )
    {
        ...
        for ( int i = 0; i < listaCadenas.size(); i++) {
        	String caracter = listaCadenas.get(i).toLowerCase();
    ...
    public int contarEnterosRepetidos() {
	    int repetidos = 0;
	    List<Integer> numerosVistos = new ArrayList<>();
	
	    for (int i = 0; i < listaEnteros.size(); i++) {
	        int elementoActual = listaEnteros.get(i);
	        
	        if (numerosVistos.contains(elementoActual)) {
	            continue; 
	        }
	
	        for (int j = i + 1; j < listaEnteros.size(); j++) {
	            if (elementoActual == listaEnteros.get(j)) {
	                repetidos++;
	                numerosVistos.add(elementoActual); 
	                break; 
	            }
	        }
	    }
    ...
    public void generarEnteros( int cantidad, int minimo, int maximo )
    {
		listaEnteros.clear(); 
		    if (minimo > maximo) {
		        int limite_inferior= minimo;
		        minimo = maximo;
		        maximo = limite_inferior;
		    }
